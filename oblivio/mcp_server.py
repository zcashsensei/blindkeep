"""Oblivio as an MCP server: any MCP host becomes an AI with a keep.
...
INSTRUCTIONS = (
    "Oblivio is this user's encrypted memory. Records carry a sensitivity "
    ...
    "the user in the Oblivio app. Save memories at the honest class — "
    "'secret' is write-only for you: you can store it, and only the user "
    "will ever read it back."
)

HOW_TO_CONNECT = (
    "The Oblivio app is not reachable. Ask the user to start it "
    "(py -3 app.py in the Oblivio folder, or the desktop launcher) — "
    "the app runs the keep this server bridges to."
)

HOW_TO_ENABLE = (
    "No agent token is configured, so Oblivio has not been told to trust "
    "this connection. Ask the user to: open the Oblivio app "
    "(http://127.0.0.1:8743), switch on Agent access, copy the token it "
    "mints, and set it as OBLIVIO_AGENT_TOKEN in this MCP server's "
    "environment. Handing an agent memory access is the user's decision; "
    "nothing on this side can grant it."
)

HOW_TO_REFRESH = (
    "Oblivio refused the agent token. Either agent access was switched "
    "off, or the app restarted and minted a new token. Ask the user to open "
    "the Oblivio app, check Agent access is on, and update "
    "OBLIVIO_AGENT_TOKEN in this MCP server's config to the current token."
)
...
class OblivioBridge:
    ...
    def __init__(self, url: Optional[str] = None, token: Optional[str] = None,
                 timeout: float = 30.0):
        self.url = (url or os.environ.get("OBLIVIO_APP_URL")
    ...
    def token(self) -> str:
        ...
        return self._token or os.environ.get("OBLIVIO_AGENT_TOKEN") or ""

    def request(self, method: str, path: str,
                body: Optional[dict] = None) -> tuple[int, dict]:
        data = None
        headers = {"Accept": "application/json",
                   "X-Oblivio-Token": self.token}
        if body is not None:
...
def _refused(status: int, detail: dict) -> dict:
    ...
    if status == 409 and detail.get("code") == "key_not_backed_up":
        return _text(
            ...
            "the user to finish key setup in the Oblivio app. Do not "
            "bypass this.", is_error=True)
    return _text(f"Oblivio refused: {err}", is_error=True)


def call_tool(bridge: OblivioBridge, name: str, args: dict) -> dict:
    if name == "save_memory":
        if not bridge.token:
            return _no_token()
        text = args.get("text")
    ...
    if name == "keep_status":
        ...
        if not bridge.token:
            ...
            return _text("The Oblivio app is running at " + bridge.url
...
def _handle(bridge: OblivioBridge, msg: dict) -> Optional[dict]:
    ...
    if method == "initialize":
        ...
        return result({
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "oblivio", "title": "Oblivio",
                           "version": __version__},
    ...
    if method == "tools/call":
        ...
        try:
            ...
            out = _text(f"oblivio bridge error: "
...
def serve_stdio(url: Optional[str] = None, token: Optional[str] = None) -> int:
    ...
    bridge = OblivioBridge(url=url, token=token)
    print(f"oblivio mcp: bridging to {bridge.url} "
