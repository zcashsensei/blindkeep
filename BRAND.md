...
The word is solid. Under it sits the proof: `1d5e3e4a5fb29328`, the Merkle root the
README's succinct-membership demo prints — real, published and checkable, rather than hex
chosen to look cryptographic.

It is set **beneath** the word rather than inside it. An earlier version clipped the hex
into the letterforms; it read as texture instead of evidence, because at README width the
digits were far too small to make out. **A mark that shows an unreadable proof is
decoration claiming to be a guarantee** — the same failure as a status page that is always
green. Solid word, legible root.
