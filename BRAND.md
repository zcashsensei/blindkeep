...
The letterforms are not solid. They are a clipping path, and what shows through them is a
real Merkle root — `1d5e3e4a5fb29328`, the one the README's membership demo prints —
repeated with each row offset so the columns do not draw seams down the glyphs. The mark
is made of the artefact the project produces, and the digits are checkable against a
published value rather than invented to look cryptographic. A faint solid backing sits
under the hex at 28% so the letterforms hold their shape at README scale; set `BACKING`
to 0 in the generator to see why it is there.
