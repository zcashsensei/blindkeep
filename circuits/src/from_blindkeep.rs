//! Prove a witness exported by Blindkeep — the two halves, actually connected.
//!
//! Everything else in this repository proves claims about the trading system. This proves that the
//! *memory layer* and the *circuit* agree: a witness produced by `blindkeep zk-witness`, over a
...
mod tests {
    ...
    /// Big-endian hex -> Fp. The Blindkeep side emits big-endian so this is a straight read.
    ...
    #[test]
    fn a_blindkeep_witness_produces_a_real_proof() {
        let path = match std::env::var("BLINDKEEP_WITNESS") {
            Ok(p) => p,
            ...
            Err(_) => { println!("  BLINDKEEP_WITNESS not set — skipping"); return; }
        };
        ...
        assert!(mock.verify().is_ok(), "the Blindkeep witness does not satisfy the circuit");
        ...
        println!("  PROOF FROM A BLINDKEEP RECORD: {} bytes", proof.len());
